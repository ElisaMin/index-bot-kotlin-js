...
internal class ApiClient(
    ...
    fun setWebhook(
        ...
        is ByFile -> service.setWebhookWithCertificateAsFile(
            url = url.toMultipartBodyPart("url"),
            certificate = certificate.file.toMultipartBodyPart(
                partName = "certificate",
                mediaType = MediaTypeConstants.UTF_8_TEXT,
            ),
            ipAddress = ipAddress?.toMultipartBodyPart("ip_address"),
            maxConnections = maxConnections?.toMultipartBodyPart("max_connections"),
            allowedUpdates = allowedUpdates?.toMultipartBodyPart("allowed_updates"),
            dropPendingUpdates = dropPendingUpdates?.toMultipartBodyPart("drop_pending_updates"),
        )
